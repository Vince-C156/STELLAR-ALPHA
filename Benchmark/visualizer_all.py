...
class trajectory_artist:

    """Object for matplotlib line artist containing trajectory data for a single unique run

    Init Args:
        line_artist (_type_): Matplotlib line artist from ax.plot
        X (_type_): Array of x positions
        Y (_type_): Array of y positions
        Z (_type_): Array of z positions
        file_id (_type_): text file name
        directory (_type_): directory of benchmarks (vbar{n})
    """        
    def __init__(self, line_artist, X, Y, Z, file_id, directory):

        print(f'CREATED TRAJECTORY OBJECT FOR {file_id}')
        print('----------------------')
        print(f'line_artist: {line_artist}')
        print(f'type(line_artist): {type(line_artist)}')

        self.line_artist = line_artist[0]
        self.X = X
        self.Y = Y
        self.Z = Z
    ...
    def __repr__(self):
        return f"Trajectory(file='{self.file_id}', directory={self.directory})"
class render_visual:
    ...
    def color_fade_with_y(self, X, Y, Z, cmap='coolwarm'):
        ...
        surf = self.ax.plot_surface(X, Y, Z, cmap=cmap, facecolors=face_colors, rstride=5, cstride=5, linewidth=0.7, alpha=0.2, antialiased=False)
    ...
    def create_artists(self):
        ...
        """Initalize n artists for n trajectories from each unique run

        Args: No args

        Returns: returns a list of trajectory_artist objects
        """

        cycol = cycle('rgbycmk')
        all_artists = []

        #for a folder of runs in each single run  (chaser0.txt, chaser1.txt, chaser2.txt) get all the data from a single run for each axis
        for run_file in os.listdir(self.data_path):
            ...
            artist = self.ax.plot(xs[0], ys[0], zs[0], c=next(cycol), linewidth=0.7, alpha=0.5, markevery=15, antialiased=False)
            all_artists.append(trajectory_artist(line_artist=artist, X=xs, Y=ys, Z=zs, file_id=run_file, directory=self.data_path))

        return all_artists
    
    def init_artists(self):

        """Calls create_artists() to initalize all artists and then sets self.artists to the list of artists from create_artists()
        ...
        print(f'INITALIZED {len(self.artists)} ARTISTS')


    def update_artists(self, i):

        """Updates all artists to the ith position in their respective trajectories

        Args: 
            i (_type_): current timestep
        ...
        for artist in self.artists:
            print(repr(artist))
            #artist.line_artist.set_xdata(artist.X[:i])
            #artist.line_artist.set_ydata(artist.Y[:i])
            #artist.line_artist.set_3d_properties(artist.Z[:i])

            artist.line_artist.set_data_3d(artist.X[:i], artist.Y[:i], artist.Z[:i])
    ...
    def animate(self, i, file_name = None):
        ...
        for run_file in os.listdir(self.data_path):
            ...
            for idx, line in enumerate(lines):
                ...
                if idx > 1 and self.only_inital and self.term_plots == False:
                    ...
                    if self.center_termin != True:
                        ...
                        #-500 500 , -440, 560, -500, 500
                        xmin, xmax = 0 - self.center_zoom, 0 + self.center_zoom
                        ymin, ymax = 60 - self.center_zoom, 60 + self.center_zoom
                        zmin, zmax = 0 - self.center_zoom, 0 + self.center_zoom
            ...
            if self.term_plots == False:
                if self.run_file != None:
                    ...
                    self.ax.plot(xs[:i], ys[:i], zs[:i], c=next(cycol), linewidth=0.7, alpha=0.5, markevery=15, antialiased=False)
            else:
                term_scatter = self.ax.scatter3D(xs[-1], ys[-1], zs[-1], c='b', s=0.5)
                ...
                dock_scatter = self.ax.scatter3D([0], [60], [0], c='r', s=1.5)
            ...
            if self.run_file != None and zoom_los == None and self.term_plots == False:
                #zoom_los = input('Zoomed in view for isolated LOS visual [Y/N] : ')
                #break
                #if zoom_los == 'Y':
        ...
        if self.center_termin == True:
            pass
            #self.ax.set_xticks(np.arange(-200, 200, 40))
            #self.ax.set_yticks(np.arange(-140, 260, 40))
            #self.ax.set_zticks(np.arange(-200, 200, 40))
    ...
    def render_animation(self, file_name = None):
        ...
        ani = animation.FuncAnimation(self.fig, self.update_artists, repeat=False, cache_frame_data=True, interval=10)
        #print(ani.to_jshtml())
        #plt.ion()
        ...
        #plt.show()
        #sleep(1.0)
        ani.save('trajectories.gif', writer='imagemagick', fps=30)
        print('SAVED ANIMATION as trajectories.gif')
