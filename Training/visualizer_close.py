...
class render_visual:
    ...
    def __init__(self, data_dir = 'runs'):
        ...
        self.fig.subplots_adjust(top=1.01, bottom=-.1)
        ...
        #u, v = np.meshgrid(X_los, Y_los)
        #tri = mtri.Triangulation(u, v)
        self.ax.scatter(X_los, Y_los, Z_los, s=10, c=x[::-1], alpha=0.19, cmap=cm.coolwarm, vmin=-1.0, vmax=1.0)
        #self.ax.plot_trisurf(X_los, Y_los, Z_los, triangles=tri, cmap=cm.jet, linewidth=0.1)
    ...
    def data_for_cone_along_y(self, center_x, center_z, theta, height_y):
        Y = np.linspace(height_y+1, 800, 2000)
    ...
    def animate(self, i, file_name):
        ...
        self.ax.scatter(xs[1], ys[1], zs[1], marker='o', s=40)

        colors = ["white", "lightgreen", "green", "darkgreen", "purple"]
        cmap1 = LinearSegmentedColormap.from_list("mycmap", colors)

        fade = np.exp(np.linspace(-10.0, 10.0, i))
        self.ax.scatter(xs[:i], ys[:i], zs[:i], marker='o', s=0.01, c=fade[::], cmap=cmap1, vmin=-10.0, vmax=10.0)
    ...
    def render_animation(self, file_name):
        ...
        ani = animation.FuncAnimation(self.fig, partial(self.animate, file_name=file_name), frames=1000, repeat=False, cache_frame_data=False, interval=1,blit=False)


        plt.ion()
        plt.show(block=False)

        mp4_writer = animation.FFMpegWriter(fps=60)
        ani.save('animation.mp4', writer=mp4_writer)
        #sleep(2.0)
